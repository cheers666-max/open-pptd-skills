...
2. The script searches image backends (`baidu`, `openverse`, `wikimedia`, `vertical`; auto-cascaded), downloads candidates, ranks them (landscape-first based on element `bounds` ratio; VLM visual judging when `PPT_API_KEY` / `QIHOO_360_API_KEY` / `QIHOO_API_KEY` is set, `--no-vlm` to skip), saves the winner to `media/`, and rewrites the `.page` `src:` lines in place to local relative paths.
...
10. Static HTML export (optional deliverable): use `scripts/export_html.py` to produce a `html/` folder next to the deck. It renders through the skill's own deterministic HTML5 renderer (`scripts/viewer.html`) via headless Chrome, with no network access:
