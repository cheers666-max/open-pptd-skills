...
> **Keep the page out of the conversation.** Extract what you need where the bytes already are — `grep`/`sed`/a short python filter that prints the handful of lines you are after, never a whole HTML page or long document echoed back into the reply. A bloated conversation is not a slow conversation, it is a broken one: past a certain size the model stops dispatching tools and starts printing them as prose, and every following turn inherits that same context. Fifty lines of output per lookup is a generous ceiling.
...
Caption from what the pool actually recorded: `images_pool.json` carries the backend, source URL and licence for every picture. **Every inset picture gets a caption** directly under it saying what the picture shows — 「商丘古城城墙与城湖」, 「金缕玉衣（河南博物院藏）」. Only that: the source is bookkeeping the page already carries on its 来源 line, and repeating it under every figure doubles the caption and crowds out the words that explain the image. `caption-source-tail` blocks a 「· 机构 · 年份」 tail. **The caption itself is plain text — no anchor.** A row of underlines under every figure pulls the eye off the pictures they explain, so the clickable citation lives on the page's 来源 line instead, where a reader looking for the evidence goes; the pool's `landing` URL belongs there (or in the notes), not under the picture. `caption-hyperlink` blocks a link inside a caption. Never a retrieval date, never "许可未知", never a bare "资料图" — and if the pool entry cannot say where the picture came from at all, drop the picture rather than put an unattributable one on the page; the 来源 line still has to name something real. Full-height art that runs off a page edge is decoration and needs no caption.
...
2. **Read the rendered result.** With image input, inspect `.qa-images/overview.jpg` first, then the
   **compressed copy** of each suspicious page — `.qa-images/review/page_NN.jpg` (1280 px, ~57 KB),
   ...
   - **每个图片槽位在 outline 里声明 `ratio` 与 `subject`**，一次声明决定三件事：
     `ratio`（版位的宽/高）决定检索方向与比例匹配加分；`subject` 决定 fit 模式——`person`／`artifact` 用 `contain`（整体入画，宁可留白），`scene`／`diagram` 用 `cover`（填满）。
   ...
   - **每张嵌入式配图都要有图注**，紧贴图片下方，只写"这是什么"（「商丘古城城墙与城湖」）；整幅出血的装饰图除外。**图注里不写出处**——出处是台账，页面底部的「来源：」行已经承担了，跟在每张图后面只会把图注撑长、挤掉真正解释画面的字。缺图注由 `image-missing-caption` 拦，出处尾巴由 `caption-source-tail` 拦。
