...
Parallel writing is optional and host-dependent. Each worker reads this same context and the relevant PPTD guidance, receives its assigned pages and available media, and writes only those pages. With no subagent tool, follow the same plan sequentially using the quickstart's shared setup and 2–3-page modules. Check the first module through `prepare_deck.py` after resolving its images; after each batch of fixes, repeat it and inspect the updated full overview. Keep generator/pages in sync and retain local media with provenance. Module boundaries and cached PNGs do not remove planned content, generation rounds or the final whole-deck review. Do not automatically run the separate seven-dimension benchmark judge; this does not waive content review, visual review, repair or recheck.
