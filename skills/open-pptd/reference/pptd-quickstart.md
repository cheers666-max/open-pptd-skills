...
Constructors accept native PPTD fields: `text(id,x,y,w,h,value,**content)`, `shape(id,x,y,w,h,name='rect',**fields)`, `icon(id,x,y,w,h,name,**fields)`, `line(id,x,y,w,h,points,**fields)`. `page` accepts `page_type`, `background`, `notes`; `write_project` accepts `theme`, `size`, and explicit `overwrite=True` for regenerating your own draft. Existing-deck edits should preserve original files. Element types without helper constructors can be ordinary dictionaries; look up their supported specification first. Pages must contain a flat `elements` list of objects with unique nonempty IDs, supported types and finite `[x,y,w,h]` bounds (nonnegative sizes). `write_project` checks every page before writing; never hide malformed items by flattening or dropping them. The helper chooses no layouts, content or sources. Add permitted search fallback comments to the relevant `.page` src lines after serialization.
...
2. Write modules of **2–3 pages**, e.g. `pages_01_03.py`, `pages_04_06.py`, each returning its page dictionaries. Keep each page's argument, notes, evidence, layout and visual explanation specific. A thin `build_deck.py` assembles completed modules in order and calls `write_project`. Run it after the first module, resolve that draft’s image sources and run `prepare_deck.py` immediately. Inspect this early render before expanding its pattern, then extend to the full planned page count; use `overwrite=True` only to regenerate your own draft.
3. Every file-write call must contain both a complete explicit `path` and complete `content`; supply the destination first when forming the call. If a write fails or is truncated, inspect that destination and resend only the affected module with both arguments. Reuse successful modules; do not resend the whole deck or reduce its content. After a module repair, rerun the assembler and the image resolver before checking; keep local media and `images_report.json` so matching images can be reused. A direct `.page` fix must also reach its generator before a later rebuild. Existing-deck edits retain their organization and scope.

Use `python3 <skill-dir>/scripts/prepare_deck.py /abs/project/deck.pptd` for the existing validation, rendering and auxiliary audit. After repairs, rerun that command: only changed pages/dependencies rerender, but every current page remains in the overview and audits. Use `--force` for browser/system-font changes, not routine edits. After the applicable content/visual review, `--export html,pptx` adds the default deliverables; inspect export warnings and actual files. Treat estimated layout warnings as candidates for rendered inspection, not a reason to build another DOM/pixel checker or iterate until a heuristic count is zero. Text-only authors follow the SKILL's explicit fallback and preserve current images for independent visual review; a textual image-read result or auxiliary report is not seeing the slide. Do not add a separate framework, rasterize the whole deck, or skip sources or visual review to reduce tool calls. The separate seven-dimension benchmark scoring is opt-in and is not part of this generation/repair loop.
