...
- [x] T025 精简技能重复上下文，接入早期模块检查与批量修复；保留生成内评审修复，默认不自动运行七维独立评分。首模块、单页修复/恢复素材、文字模型交付三类技能场景只读复测通过；不冒充真实新模型评测。
- [x] T026 完成本轮回归、真实通用样页、技能场景与打包验证；逐项提交并推送 GitHub 的 360-intranet 分支。51 项 Node、75 项主 Python、41 项图片测试通过；7 个实施/验证提交已推送并核对远端 `670f529`，本条为完成记录。
